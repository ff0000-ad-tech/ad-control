	...
	*/ 
	load(callback, onFail) {
		...
		if ( I._isLoading ){
			...
			I._pendingImages = [];
			I._imageManagerLoader.add(new Loader(currentPendingImages, { 
			...
			I._pendingCanvasImages = [];
			I._imageManagerLoader.add(new Loader(currentPendingCanvasImages, { 
	...
	addFbaImages(target) {
		if (target)
			this._addToDictionary(target.getAllContentRaw());
	}
