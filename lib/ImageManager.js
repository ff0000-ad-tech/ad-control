...
export function get(imageId) {
  if (_dict[imageId]) return _dict[imageId];
    ...
    throw new Error(
      "ImageManager : No image named '" + imageId + "' has been loaded"
    );
}
  ...
  if (exists) {
    ...
    if (internal)
      console.log(
        ...
        ': Duplicate Image Id "' +
          imageId +
          '". One or more images loading in have the same name. Each Image needs a unique file name.'
      );
  } else {
    ...
    if (!internal)
      console.log(
        "ImageManager.available() -->",
        false,
        ": No image named '" + imageId + "' has been loaded"
      );
  }
