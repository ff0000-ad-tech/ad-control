...
class CssManager {
	...
	*/
	setDebugFilter(filter) {
		const C = this
		trace('CssManager.setDebugFilter(),', filter);
		C.filter = filter;
		C.debug_level1 = true;
	}
	ifDebug(debugLevel) {
		...
		if (!C.filter)
			return C[debugLevel];
		else if (C.passDebugFilter() && C[debugLevel])
			return true;
	}
	passDebugFilter() {
		const C = this
	...
	apply(element, cssList) {
		...
		if (C.ifDebug('debug_level1')) trace('  CssManager.apply()', element.id);
		...
		for (var key in cssList) {
			...
			if (key.match( /^transform\(/ ))
				transformList[key] = cssList[key]
			// standard css-key
			else {
				if (C.ifDebug('debug_level1')) trace('   ' + key + ': ' + cssList[key] + ';');
	...
	conformCssObject(cssObject, debugElement) {
		...
		if(C.ifDebug('debug_level1')) trace('CssManager.conformCssObject()', cssObject);
		var cssList = {};
		if (cssObject) {
			for (var key in cssObject) {
				if (C.ifDebug('debug_level2')) trace('  PARSE( key: ' + key + ', value: ' + cssObject[key] + ' )');
	...
	applyTransforms(element, value) {
		...
		if (C.ifDebug('debug_level1')) 
			trace('    - CssManager.applyTransforms(), ', value);
	...
	standardizeKey(key) {
		...
		if (key in C.keyFormatExceptions()) 
			key = C.keyFormatExceptions()[key];

		// or procedurally convert to camel
		else key = C.camelateKey(key);

		if (C.ifDebug('debug_level2')) trace('    - result: "' + key + '"');
		return key;
	}
	camelateKey(key) {
	...
	conformValue(key, value) {
		...
		else if (key == 'transform') { // && Device.browser == 'ie') {
